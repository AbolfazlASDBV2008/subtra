document.addEventListener('DOMContentLoaded', () => {
    ...
    function unmaskTags(text, tags) {
        ...
        // تبدیل اعداد فارسی/عربی به انگلیسی برای جلوگیری از باگ‌های توهم هوش مصنوعی (مثل تگ_۰)
        ...
        let unmasked = normalizedText.replace(/[_\[\]\*\-\s]*(?:TAG|tag|تگ|Tag)[_:\-\s]*(\d+)[_\[\]\*\-\s]*/gi, (match, index) => {
        ...
        // Fallback: اگر تگی کلاً از متن حذف شده بود، آن را به ابتدای خط اضافه کن تا استایل خراب نشود و وسط کلمات نیفتد
        let missingTags = "";
    ...
    // [!!!] پرامپت جدید با تمرکز بر جلوگیری از توهم و تکرار [!!!]
...
1. **تشخیص دقیق فاعل و مفعول:**
   - در جملات انگلیسی، دقت کن چه کسی کار را انجام می‌دهد.
   - مثال: "I'm counting on you" باید بشود "روت حساب می‌کنم".

2. **حفظ قالب و عدم تکرار (بسیار مهم):**
   - هرگز یک خط را دو بار ترجمه و تکرار نکن. (DO NOT duplicate or repeat lines).
   - ساختار {شروع}{پایان} را دقیقاً در ابتدای هر خط حفظ کن.

3. **ترجمه اصطلاحات:**
   - معادل فارسی پیدا کن. "No way" -> "امکان نداره".

4. **مدیریت تگ‌های سیستمی (حیاتی):**
   - در متن ورودی کدهایی شبیه به ___TAG_0___ یا ___TAG_1___ وجود دارد. 
   - به هیچ وجه آن‌ها را ترجمه نکن، فرمتشان را تغییر نده.
   - تگ‌های جدید از خودت اختراع نکن و آن‌ها را دقیقاً به همان شکل در خروجی قرار بده.
    `.trim();
    ...
    function handleFiles(files) {
        const newFiles = Array.from(files).filter(file => {
            ...
            if (file.size > 50 * 1024 * 1024) { 
                showError(`فایل "${escapeHTML(file.name)}" (${(file.size / 1024 / 1024).toFixed(1)}MB) از حد مجاز 50MB حجیم‌تر است.`);
                return false;
            }
            return true;
        });
        ...
        if (aiOutputMicroDVD) {
            ...
            for (const line of aiLines) {
                ...
                if (match) {
                    ...
                    // [!!!] جلوگیری از توهم تکرار: اگر قبلا این خط رو داشتیم، دومی (که کپی و توهم AI هست) رو کلاً نادیده بگیر [!!!]
