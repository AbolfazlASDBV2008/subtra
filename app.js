document.addEventListener('DOMContentLoaded', () => {
    ...
    function unmaskTags(text, tags) {
        ...
        let unmasked = text.replace(/[_\[\]\*\-\s]*(?:TAG|tag|تگ|برچسب)[_:\-\s]*(\d+)[_\[\]\*\-\s]*/gi, (match, index) => {
    ...
    // [!!!] اضافه کردن المان داینامیک برای چک‌باکس نادیده گرفتن استایل ASS
    const forceDefaultStyleContainer = document.createElement('div');
    forceDefaultStyleContainer.id = 'forceDefaultStyleContainer';
    forceDefaultStyleContainer.className = 'mt-3 hidden';
    forceDefaultStyleContainer.innerHTML = `
        <label class="flex items-center text-sm text-gray-300 bg-gray-700 p-2 rounded cursor-pointer hover:bg-gray-600 transition-colors border border-gray-600">
            <input type="checkbox" id="forceDefaultAssStyle" class="mr-3 rounded bg-gray-800 border-gray-600 text-blue-500 focus:ring-blue-600 w-4 h-4"> 
            <span class="leading-snug">نادیده گرفتن استایل‌های فایل ASS ورودی <br> <span class="text-xs text-gray-400">(اعمال استایل پیش‌فرض و زیبای پروژه روی این فایل)</span></span>
        </label>
    `;
    if(outputFormatSelector) {
        outputFormatSelector.appendChild(forceDefaultStyleContainer);
    }
    ...
    // [!!!] پرامپت جدید با تمرکز بر ضمایر، دقت ترجمه و تگ‌های صحیح [!!!]
...
۱. در متن ورودی کدهایی شبیه به ___TAG_0___ وجود دارد. به هیچ وجه آن‌ها را ترجمه نکن، فرمتشان را تغییر نده و دقیقاً کنار معادل فارسی کلمه‌شان قرار بده.
۲. توهم تگ: فقط و فقط از تگ‌هایی که در خط ورودی می‌بینی استفاده کن. به هیچ وجه تگ جدیدی از خودت اختراع نکن و تگ‌های خطوط دیگر را با هم قاطی نکن.
۳. عدم تکرار: جملات ترجمه شده را به هیچ وجه دو بار تکرار نکن (DO NOT duplicate or repeat the translated sentences). هر خط را فقط یک بار ترجمه کن.
    ...
    function updatePromptUI() {
        ...
        if (currentPromptId === 'default') {
            ...
            systemPrompt.classList.add('bg-gray-700', 'opacity-75'); 
    ...
    function handleFiles(files) {
        ...
        if (uploadedFiles.length > 0) {
            ...
            // بررسی وجود فایل ASS برای نمایش گزینه نادیده گرفتن استایل
            let hasAss = uploadedFiles.some(f => f.name.toLowerCase().endsWith('.ass'));
            if (forceDefaultStyleContainer) {
                if (hasAss) forceDefaultStyleContainer.classList.remove('hidden');
                else forceDefaultStyleContainer.classList.add('hidden');
            }
    ...
    clearFileList.addEventListener('click', () => {
        ...
        if (forceDefaultStyleContainer) {
            forceDefaultStyleContainer.classList.add('hidden');
            const forceCb = document.getElementById('forceDefaultAssStyle');
    ...
    // [!!!] FIX: Update logic to use Array mapping to completely prevent AI duplication hallucinations [!!!]
    function mergeTrustedFramesWithAiText(originalMicroDVD, aiOutputMicroDVD) {
        ...
        // استفاده از Map برای ذخیره آرایه‌ای از ترجمه‌ها (جلوگیری از توهم و تکرار)
            ...
            for (const line of aiLines) {
                ...
                if (match) {
                    ...
                    if (translatedTextMap.has(timeBlockKey)) {
                        translatedTextMap.get(timeBlockKey).push(text);
                    } else {
                        translatedTextMap.set(timeBlockKey, [text]);
                    }
                } else if (lastSeenKey) {
                    const currentArr = translatedTextMap.get(lastSeenKey);
                    if (currentArr && currentArr.length > 0) {
                        const lastIdx = currentArr.length - 1;
                        currentArr[lastIdx] = currentArr[lastIdx] + " " + trimmedLine;
                    }
                }
            }
        }
        ...
        for (let i = 0; i < originalLines.length; i++) {
            ...
            if (match) {
                ...
                if (translatedTextMap.has(timeBlockKey) && translatedTextMap.get(timeBlockKey).length > 0) {
                    // برداشتن دقیقاً یک ترجمه. اگر هوش مصنوعی توهم زده باشه و ۱۰ تا ترجمه برای یک فریم داده باشه، فقط اولی رو برمیداره و بقیه نادیده گرفته میشن
                    const translatedText = translatedTextMap.get(timeBlockKey).shift();
    ...
    async function performSelfCorrection(texts, fileIndex, model, apiKey, prompt) {
        ...
        for (let i = 0; i < texts.length; i++) {
            ...
            // [!!!] FIX: فقط در صورتی انگلیسی رو خطا بگیر که هیچ کاراکتر فارسی داخلش نباشه [!!!]
            const hasPersian = /[\u0600-\u06FF]/.test(textForCheck);
            ...
            // اگر هیچ حرف فارسی نداره و توش انگلیسی هست (یعنی کلا ترجمه نشده)، یا حروف خراب داره
            if ((!hasPersian && hasEnglish) || hasForeign || hasBadChars) {
    ...
    startTranslation.addEventListener('click', async () => {
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
            ...
            try {
                ...
                // [!!!] اضافه شدن منطق چک کردن گزینه "نادیده گرفتن استایل ASS"
                const forceDefaultStyleCb = document.getElementById('forceDefaultAssStyle');
                const forceDefaultStyle = forceDefaultStyleCb ? forceDefaultStyleCb.checked : false;

                // اگر فایل ASS بود، کاربر خروجی ASS خواست و تیک نادیده گرفتن استایل رو "نزد"، استایل اصلی حفظ میشه
                useAssPath = file.name.toLowerCase().endsWith('.ass') && outputFormatChoice === 'ass' && !forceDefaultStyle;

                if (useAssPath) {
                    addLog(`فایل ${file.name} به عنوان ASS (با حفظ استایل اصلی) پردازش می‌شود.`);
                    ...
                    if (file.name.endsWith('.srt')) {
                        ...
                        if (forceDefaultStyle) {
                            addLog(`گزینه نادیده گرفتن استایل فعال است. فایل ${file.name} با اعمال استایل پیش‌فرض پردازش می‌شود.`);
                        } else {
                ...
                if (useAssPath) {
                     ...
                     if (aiDetectionToggle.checked) {
                         mainMicroDVD = linesObj
                            ...
                            .filter(l => !masterTranslationMap.has(l.time)) 
                            .map(l => l.line).join('\n');
